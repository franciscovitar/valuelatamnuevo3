...
function animatePersonCard(card, ctx, direction, cleanups) {
  ...
  tl.to(role, { opacity: 1, y: 0, duration: 0.682, ease: SECTION_MOTION.ease }, 0.14);
  if (nameInner) {
    tl.to(nameInner, { y: 0, duration: SECTION_MOTION.duration, ease: SECTION_MOTION.ease }, 0.22);
  }
  if (bioLines.length) {
    tl.to(bioLines, { opacity: 1, y: 0, duration: SECTION_MOTION.duration, stagger: 0.1, ease: SECTION_MOTION.ease }, 0.34);
  }
  if (link) {
    tl.to(link, { opacity: 1, y: 0, duration: 0.228, ease: SECTION_MOTION.ease }, 0.52);
...
export function initTeamSection(root, ctx, cleanups) {
  ...
  // Corte editorial azul noche.
  pushCleanup(cleanups, initBandWipe(section, ctx, { color: 'var(--p-ink-900)' }));
