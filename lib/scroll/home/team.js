...
function animatePersonCard(card, ctx, direction, cleanups) {
  ...
  if (nameInner) {
    tl.to(nameInner, { y: 0, duration: 0.6, ease: 'power2.out' }, 0.22);
  }
  if (bioLines.length) {
    tl.to(bioLines, { opacity: 1, y: 0, duration: 0.6, stagger: 0.1, ease: 'power2.out' }, 0.34);
  }
  if (link) {
    tl.to(link, { opacity: 1, y: 0, duration: 0.28, ease: 'power2.out' }, 0.52);
