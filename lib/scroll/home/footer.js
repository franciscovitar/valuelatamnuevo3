...
export function initFooterSection(ctx, cleanups) {
  ...
  if (logo) {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(logo, { opacity: 1, y: 0, duration: 0.68, ease: 'power2.out' });
      },
    });
  ...
  const colSt = ScrollTrigger.create({
    ...
    once: true,
    onEnter: () => {
      gsap.to(columns, { opacity: 1, y: 0, duration: 0.68, stagger: 0.09, ease: 'power2.out' });
    },
  });
  ...
  if (bottom) {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(bottom, { opacity: 1, duration: 0.68, ease: 'power2.out' });
      },
    });
    pushCleanup(cleanups, () => st.kill());
  }
  ...
  links.forEach((link) => {
    ...
    const onEnter = () => {
      ...
      gsap.to(underline, { scaleX: 1, duration: 0.224, ease: 'power2.out' });
    };
    const onLeave = () => {
      gsap.to(link, { x: 0, duration: 0.18, ease: 'power2.out' });
      gsap.to(underline, { scaleX: 0, duration: 0.222, ease: 'power2.in' });
    };
