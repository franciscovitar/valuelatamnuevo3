...
export function initFooterSection(ctx, cleanups) {
  ...
  if (logo) {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(logo, { opacity: 1, y: 0, duration: 0.6, ease: 'power2.out' });
      },
    });
  ...
  const colSt = ScrollTrigger.create({
    ...
    once: true,
    onEnter: () => {
      gsap.to(columns, { opacity: 1, y: 0, duration: 0.6, stagger: 0.09, ease: 'power2.out' });
    },
  });
  ...
  if (bottom) {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(bottom, { opacity: 1, duration: 0.6, ease: 'power2.out' });
      },
    });
    pushCleanup(cleanups, () => st.kill());
  }
