...
export function initContactSection(root, ctx, cleanups) {
  ...
  if (form) {
    ...
    formTl.to(rows, { opacity: 1, y: 0, duration: 0.6, stagger: 0.08, ease: 'power2.out' }, 0.12);
    if (submit) pushCleanup(cleanups, bindGoldSweep(submit));

    form.querySelectorAll('input, select, textarea').forEach((field) => {
      const onFocus = () => gsap.to(field, { boxShadow: '0 0 0 1px rgba(196, 194, 190, 0.35)', duration: 0.22 });
