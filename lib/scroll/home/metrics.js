...
export function initMetricsSection(root, ctx, cleanups) {
  ...
  metrics.forEach((metric, index) => {
    ...
    if (numWrap) {
      metricsTl.to(numWrap, { opacity: 1, y: 0, duration: 0.6, ease: 'power2.out' }, at);
    }

    if (valueEl) {
      metricsTl.add(() => {
        runCountUp(valueEl, target, ctx, { duration: 0.6 });
      }, at + 0.1);
    }

    if (cap) {
      metricsTl.to(cap, { opacity: 1, y: 0, duration: 0.6, ease: 'power2.out' }, at + 0.08);
    }
  });
