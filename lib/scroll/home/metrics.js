...
export function initMetricsSection(root, ctx, cleanups) {
  ...
  metrics.forEach((metric, index) => {
    ...
    if (numWrap) {
      metricsTl.to(numWrap, { opacity: 1, y: 0, duration: SECTION_MOTION.duration, ease: SECTION_MOTION.ease }, at);
    }

    if (valueEl) {
      metricsTl.add(() => {
        runCountUp(valueEl, target, ctx, { duration: SECTION_MOTION.duration });
      }, at + 0.1);
    }

    if (cap) {
      metricsTl.to(cap, { opacity: 1, y: 0, duration: SECTION_MOTION.duration, ease: SECTION_MOTION.ease }, at + 0.08);
    }
  });
