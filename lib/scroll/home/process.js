...
/*
 * Las opacidades del paso y las de su contenido se multiplican: un step en
 * 0.4 con copy en 0.1 daba 0.04 efectivo, que no se lee como "todavía no te
 * toca" sino como un error de renderizado. Los pasos futuros tienen que verse
 * atenuados pero presentes.
 *
 * Los tres pasos estan uno al lado del otro, no en secuencia vertical: bajarlos
 * y encogerlos los desalineaba respecto del activo y la fila entera se leia
 * rota, como si el tercero no hubiera terminado de cargar. El foco lo da la
 * opacidad, que alcanza; la caja se queda quieta.
 */
...
const PROCESS_INNER_STATES = {
  ...
  next: {
    number: { opacity: 0.92, y: 0 },
    title: { opacity: 0.9, y: 0 },
    copy: { opacity: 0.8, y: 0 },
  },
  future: {
    number: { opacity: 0.86, y: 0 },
    title: { opacity: 0.82, y: 0 },
    copy: { opacity: 0.7, y: 0 },
  },
};
