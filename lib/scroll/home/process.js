...
const PROCESS_INNER_STATES = {
  ...
  past: {
    number: { opacity: 0.5, y: 4 },
    title: { opacity: 0.36, y: 4 },
    copy: { opacity: 0.12, y: 4 },
  },
  next: {
...
function initProcessMobile(section, ctx) {
  ...
  if (closer) {
    revealOnce(closer, ctx, { trigger: closer, y: 16, duration: 0.96, delay: 0.06 });
  }
}
