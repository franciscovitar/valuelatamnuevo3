...
function persistSeenProcessSteps(steps, maxSeenIndex, activeIndex) {
  ...
  steps.forEach((step, stepIndex) => {
    /*
     * Solo se consolidan los pasos estrictamente anteriores al activo.
     *
     * Antes bastaba con que el paso ya se hubiera visto para forzarlo a su
     * estado final con un `gsap.set`, que es instantaneo. Como el indice activo
     * oscila alrededor de su umbral mientras el scrub persigue al scroll, al
     * primer frame en que retrocedia se pisaba el tween del paso que estaba
     * entrando y la tarjeta saltaba a opacidad 1: la entrada se veia de golpe.
     */
...
function initProcessMobile(section, ctx, cleanups) {
  ...
  /*
   * El reveal usa el helper del sistema y no un `gsap.from` suelto: ese
   * `from` aplica el estado inicial de inmediato (`immediateRender`) y depende
   * de que ScrollTrigger dispare `onEnter` para deshacerlo. Cuando el trigger
   * ya quedaba pasado en el refresh inicial, el `onEnter` no llegaba nunca y
   * las tres tarjetas se quedaban en opacidad 0 — invisibles toda la pagina.
   * `initFadeReveal` cubre justo ese caso con su `if (st.progress > 0) play()`.
   */
  pushCleanup(cleanups, initFadeReveal(steps, ctx, {
...
function initProcessDesktop(section, ctx, cleanups) {
  ...
  /*
   * La aparicion de cada tarjeta usa una duracion propia, mas larga que el
   * `crossfade` del sistema.
   *
   * `storyCrossfade` esta topado en 0.1 del recorrido y con tres pasos el
   * hueco entre uno y otro es 0.33: la tarjeta terminaba de entrar en menos de
   * un tercio del tramo y el resto era scroll sin nada, asi que se leia como un
   * encendido de golpe. Ocupando el 72% del tramo la entrada acompana al
   * scroll y queda descanso entre una y la siguiente. El `crossfade` no se
   * toca: lo comparten `storyActiveIndex` y los conectores.
   *
   * 0.85 es el techo util: con tres pasos el hueco entre entradas es 0.33, asi
   * que por encima de eso una tarjeta empezaria a entrar antes de que la
   * anterior termine y se perderia la lectura de uno por vez.
   */
  const revealDuration = segment * 0.85;
  ...
  for (let index = 1; index < steps.length; index += 1) {
    ...
    // La entrada termina donde el paso pasa a activo, igual que antes, pero
    // arranca mucho mas temprano.
    applyProcessFocus(tl, steps, index, at - revealDuration, revealDuration);
