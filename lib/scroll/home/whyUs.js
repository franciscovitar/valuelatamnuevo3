...
import { gsap, ScrollTrigger, paletteColor } from '@/lib/scroll/gsap';
import {
  initTextPaint,
  initFadeReveal,
  initLineMaskReveal,
...
export function initWhyUsSection(root, ctx, cleanups) {
  ...
  // Corte editorial azul noche, apenas mas profundo que Solutions.
  pushCleanup(cleanups, initBandWipe(section, ctx, { color: 'var(--p-ink-800)' }));
  ...
  if (quote) {
    ...
    if (signature) {
      ...
      const st = ScrollTrigger.create({
        ...
        once: true,
        onEnter: () => {
          gsap.to(signature, { opacity: 1, y: 0, duration: SECTION_MOTION.duration, ease: SECTION_MOTION.ease, delay: 0.12 });
        },
      });
      pushCleanup(cleanups, () => st.kill());
    }
    ...
    gsap.to(quote, {
      boxShadow: `0 24px 70px -42px rgba(0, 0, 0, 0.65), 0 0 28px ${paletteColor('--p-accent-bright-rgb', 0.18)}`,
      scrollTrigger: {
        trigger: quote,
        start: 'top 68%',
        end: 'center center',
