...
function deformSample(
  ...
  if (!floatState || floatState.idleMix <= 0.001) {
    return sample.point;
  }
  ...
  const wave = (
    Math.sin(
      floatState.time * 0.72
      + routeU * 8.5
      + line.phase
    ) * 1.05
    + Math.sin(
      floatState.time * 0.31
      + routeU * 3.4
      + line.phase * 0.6
    ) * 0.55
  );
...
function buildScrollAnchors(sections, viewportHeight) {
  ...
  orderedSections.forEach((section, index) => {
    ...
    anchors.push({
      ...
      scrollY: clamp(
        Math.max(rawScroll, previous.scrollY + 1),
        0,
        maxScroll
      ),
    });
...
export function initBackgroundLines(ctx, cleanups, root) {
  ...
  const renderPaths = (
    ...
    const floatState = {
      time,
      idleMix,
      shapeFloat: geometry.settings.shapeFloat,
    };
