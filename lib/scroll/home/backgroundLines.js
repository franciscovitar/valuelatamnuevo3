...
/*
 * The progress map intentionally leaves room for the offscreen travel.
 * The line does not teleport during those ranges; it moves outside the
 * clipped viewport and later re-enters.
 */
...
export function initBackgroundLines(ctx, cleanups, root) {
  ...
  const renderProgress = (inputProgress) => {
    ...
    const progress = clamp(inputProgress, 0, 0.96);
    const travelerState = resolveGestureState(progress);

    world.style.opacity = `${travelerState.opacity}`;
  ...
  /*
   * Slightly slower than the previous pass.
   * The reference has a soft, inertial catch-up rather than a rigid lock.
   */
