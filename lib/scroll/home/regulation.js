...
import { initFadeReveal, initCharConverge } from '@/lib/motion/textEffects';
...
export function initRegulationSection(root, ctx, cleanups) {
  ...
  if (panelTitle) {
    // Título de panel, no protagonista de sección: dispersión contenida.
    pushCleanup(
      cleanups,
      initCharConverge(panelTitle, ctx, {
        maxChars: 32,
        tier: 'base',
        spread: 20,
  ...
  if (separator) {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(separator, { scaleX: 1, duration: SECTION_MOTION.duration, ease: SECTION_MOTION.ease, delay: 0.08 });
      },
    });
    ...
    if (!line) {
      ...
      // El color sale de --vl-hairline para que el tema marfil lo invierta:
      // un filete blanco translúcido es invisible sobre fondo claro.
      line.style.cssText = 'display:block;height:1px;width:100%;margin:12px 0;background:var(--vl-hairline, rgba(246,243,236,0.14));transform-origin:left center;transform:scaleX(0);';
