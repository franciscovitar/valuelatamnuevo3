...
function initFinancingSubunit(section, ctx, cleanups) {
  ...
  if (grid) {
    ...
    pushCleanup(
      cleanups,
      initDrawLine(line, ctx, { trigger: subunit, axis: 'x', duration: 0.68 }),
    );
  }

  pushCleanup(
    cleanups,
    initClipReveal(subunit, ctx, { direction: 'right', trigger: subunit, duration: 0.68 }),
  );
  ...
  if (tag) {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(tag, { opacity: 1, y: 0, duration: 0.68, ease: 'power2.out', delay: 0.08 });
      },
    });
    pushCleanup(cleanups, () => st.kill());
  }
...
function initFinancingCta(section, ctx, cleanups) {
  ...
  pushCleanup(
    cleanups,
    initClipReveal(cta, ctx, { direction: 'bottom', trigger: cta, duration: 0.68 }),
  );
