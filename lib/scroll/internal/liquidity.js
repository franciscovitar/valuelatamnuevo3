  ...
  cards.forEach((card, index) => {
    ...
    if (copy) {
      ...
      const st = ScrollTrigger.create({
        ...
        once: true,
        onEnter: () => {
          gsap.to(copy, { opacity: 1, y: 0, duration: SECTION_MOTION.duration, ease: SECTION_MOTION.ease, delay: 0.12 });
        },
      });
...
function initLiquidityCta(section, ctx, cleanups) {
  ...
  const st = ScrollTrigger.create({
    ...
    once: true,
    onEnter: () => {
      gsap.to(cta, { opacity: 1, y: 0, duration: SECTION_MOTION.duration, ease: SECTION_MOTION.ease });
    },
  });
  pushCleanup(cleanups, () => st.kill());
