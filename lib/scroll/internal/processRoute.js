  ...
  steps.forEach((step, index) => {
    ScrollTrigger.create({
      ...
      onEnter: () => {
        ...
        revealOnScroll(step.querySelector('p'), ctx, { trigger: step, y: 12, opacity: 0.54, delay: 0.06 });
      },
    });
  });

  revealOnScroll(section.querySelector('.process-route__mobile .closer'), ctx, { y: 14, opacity: 0.54 });
}
...
function initProcessRouteDesktop(section, ctx, cleanups) {
  ...
  const timings = storySegmentTimes(panels.length, { intro: 0.1, body: 1 });
  ...
  const pinConfig = pinnedStoryConfig(section, pin, {
    end: () => `+=${Math.max(getScrollAmount() * 1.08, window.innerHeight * 0.62)}`,
    scrub: STORY_SCRUB,
    onRefresh: fitStage,
    onUpdate: () => {
