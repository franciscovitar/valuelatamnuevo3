...
function playIntroLineMask(title, tl, at = 0.1) {
  if (!title) return;
  ...
  tl.to(inner, { y: 0, duration: 0.82, ease: INTERNAL_EASE_OUT }, at);
}
...
function playIntroPhrases(lede, tl, at = 0.18) {
  if (!lede) return;
  ...
  const spans = parts.map((part, index) => {
    ...
    gsap.set(span, { opacity: 0, y: 5 });
    return span;
  });
  tl.to(spans, { opacity: 1, y: 0, duration: 0.68, stagger: 0.1, ease: INTERNAL_EASE_OUT }, at);
}
...
export function runInternalAboveFoldIntro(root, ctx) {
  ...
  const backLink = targets.find((el) => el.matches('[data-vl-page-entry], .page-back, .page-back a'));
  const eyebrow = targets.find((el) => el.matches('.eyebrow'));
  const title = targets.find((el) => el.matches('h1, h2'));
  const lede = targets.find((el) =>
    el.matches('.fin-lede, .ai-lede, .pay-hero, .lead') || (el.matches('p') && !el.closest('.sec-head')),
  );
  ...
  const tl = gsap.timeline({
    defaults: { ease: INTERNAL_EASE_OUT, force3D: true, overwrite: 'auto' },
    onComplete: () => {
      targets.forEach((el) => {
  ...
  if (backLink) {
    gsap.set(backLink, { opacity: 0, x: -6 });
    tl.to(backLink, { opacity: 1, x: 0, duration: 0.62 }, 0);
  }
  if (eyebrow) {
    gsap.set(eyebrow, { opacity: 0, y: 6 });
    tl.to(eyebrow, { opacity: 1, y: 0, duration: 0.62 }, 0.04);
  }
  playIntroLineMask(title, tl, 0.1);
  playIntroPhrases(lede, tl, 0.18);
