...
import { gsap, ScrollTrigger, paletteColor } from '../gsap';
import {
  initLineMaskReveal,
...
function initReferralsJoinBlock(block, ctx, cleanups) {
  ...
  if (eyebrow) {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(eyebrow, { opacity: 1, y: 0, duration: SECTION_MOTION.duration, ease: SECTION_MOTION.ease });
      },
    });
...
function initReferralsFeatures(section, ctx, cleanups) {
  ...
  features.forEach((feat, index) => {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(feat, { opacity: 1, y: 0, duration: SECTION_MOTION.duration, ease: SECTION_MOTION.ease });
        if (dot) {
          gsap.to(dot, { opacity: 1, scale: 1, duration: 0.42, ease: SECTION_MOTION.ease, delay: 0.06 });
        }
        if (copy && !isCnv) {
          gsap.to(copy, { opacity: 1, y: 0, duration: SECTION_MOTION.duration, ease: SECTION_MOTION.ease, delay: 0.16 });
        }
      },
    });
    pushCleanup(cleanups, () => st.kill());
...
function initReferralsExperience(section, ctx, cleanups) {
  ...
  cards.forEach((card, index) => {
    ...
    pushCleanup(
      cleanups,
      initClipReveal(card, ctx, { direction, trigger: card, duration: SECTION_MOTION.duration }),
    );
    ...
    if (copy) {
      ...
      const st = ScrollTrigger.create({
        ...
        once: true,
        onEnter: () => {
          gsap.to(copy, { opacity: 1, y: 0, duration: SECTION_MOTION.duration, ease: SECTION_MOTION.ease, delay: 0.14 });
        },
      });
      pushCleanup(cleanups, () => st.kill());
    }
...
function initReferralsFaq(section) {
  const faq = section.querySelector('.faq');
  if (!faq) return;

  // FAQ items stay visible; no scroll-triggered entrance.
  gsap.set(faq.querySelectorAll('details'), { clearProps: 'opacity,transform' });
}
  ...
  const st = ScrollTrigger.create({
    ...
    onEnter: () => {
      gsap.to(cta, {
        ...
        boxShadow: `0 0 28px -26px ${paletteColor('--p-accent-bright-rgb', 0.18)}`,
      });
    },
  });
  pushCleanup(cleanups, () => st.kill());
  ...
  if (joinBlocks[1]) {
    ...
    if (eyebrow) {
      ...
      const st = ScrollTrigger.create({
        ...
        start: triggerStart(ctx),
        once: true,
        onEnter: () => gsap.to(eyebrow, { opacity: 1, y: 0, duration: SECTION_MOTION.duration, ease: SECTION_MOTION.ease }),
      });
      pushCleanup(cleanups, () => st.kill());
    }
    if (title) {
