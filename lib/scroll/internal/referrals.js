...
function initReferralsJoinBlock(block, ctx, cleanups) {
  ...
  if (eyebrow) {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(eyebrow, { opacity: 1, y: 0, duration: 0.68, ease: 'power2.out' });
      },
    });
...
function initReferralsFeatures(section, ctx, cleanups) {
  ...
  features.forEach((feat, index) => {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(feat, { opacity: 1, y: 0, duration: 0.68, ease: 'power2.out' });
        if (dot) {
          gsap.to(dot, { opacity: 1, scale: 1, duration: 0.42, ease: 'power2.out', delay: 0.06 });
        }
        if (copy && !isCnv) {
          gsap.to(copy, { opacity: 1, y: 0, duration: 0.68, ease: 'power2.out', delay: 0.16 });
        }
      },
    });
    pushCleanup(cleanups, () => st.kill());
...
function initReferralsExperience(section, ctx, cleanups) {
  ...
  cards.forEach((card, index) => {
    ...
    pushCleanup(
      cleanups,
      initClipReveal(card, ctx, { direction, trigger: card, duration: 0.68 }),
    );
    ...
    if (copy) {
      ...
      const st = ScrollTrigger.create({
        ...
        once: true,
        onEnter: () => {
          gsap.to(copy, { opacity: 1, y: 0, duration: 0.68, ease: 'power2.out', delay: 0.14 });
        },
      });
      pushCleanup(cleanups, () => st.kill());
    }
  ...
  if (joinBlocks[1]) {
    ...
    if (eyebrow) {
      ...
      const st = ScrollTrigger.create({
        ...
        start: triggerStart(ctx),
        once: true,
        onEnter: () => gsap.to(eyebrow, { opacity: 1, y: 0, duration: 0.68, ease: 'power2.out' }),
      });
      pushCleanup(cleanups, () => st.kill());
    }
    if (title) {
