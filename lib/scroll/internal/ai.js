...
function initAIGains(section, ctx, cleanups) {
  ...
  gains.forEach((gain, index) => {
    ...
    const play = () => {
      ...
      const tl = gsap.timeline({ defaults: { ease: SECTION_MOTION.ease } });
      const at = index * 0.04;

      tl.to(gain, { opacity: 1, y: 0, duration: SECTION_MOTION.duration }, at);

      if (valueInner) {
        tl.fromTo(
          valueInner,
          { y: '108%', opacity: 0.45 },
          { y: 0, opacity: 1, duration: SECTION_MOTION.duration },
          at + 0.08,
        );
      } else if (gnum) {
        tl.fromTo(gnum, { opacity: 0.4, y: 8 }, { opacity: 1, y: 0, duration: SECTION_MOTION.duration }, at + 0.08);
      }
      ...
      if (copy) {
        tl.to(copy, { opacity: 1, y: 0, duration: SECTION_MOTION.duration }, at + 0.22);
      }
    };
...
function initAIDelivery(section, ctx, cleanups) {
  ...
  if (lede) {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(lede, { opacity: 1, y: 0, duration: SECTION_MOTION.duration, ease: SECTION_MOTION.ease, delay: 0.1 });
      },
    });
    pushCleanup(cleanups, () => st.kill());
  }
  ...
  cards.forEach((card, index) => {
    ...
    if (cardCopy) {
      ...
      const st = ScrollTrigger.create({
        ...
        once: true,
        onEnter: () => {
          gsap.to(cardCopy, { opacity: 1, y: 0, duration: SECTION_MOTION.duration, ease: SECTION_MOTION.ease, delay: 0.14 });
        },
      });
      pushCleanup(cleanups, () => st.kill());
    }
  ...
  if (closer) {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(closer, { opacity: 1, y: 0, duration: SECTION_MOTION.duration, ease: SECTION_MOTION.ease });
      },
    });
    pushCleanup(cleanups, () => st.kill());
  }
...
function initAIBand(section, ctx, cleanups) {
  ...
  pushCleanup(
    cleanups,
    initClipReveal(band, ctx, { direction: 'bottom', trigger: band, duration: SECTION_MOTION.duration }),
  );
  ...
  if (btn) {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(btn, { opacity: 1, y: 0, duration: SECTION_MOTION.duration, ease: SECTION_MOTION.ease, delay: 0.16 });
      },
    });
    pushCleanup(cleanups, () => st.kill());
