...
function initAIStory(section, ctx, cleanups) {
  ...
  steps.forEach((step, index) => {
    ScrollTrigger.create({
      trigger: step,
      start: ctx.mobile ? 'top 78%' : 'top 72%',
      end: ctx.mobile ? 'bottom 22%' : 'bottom 28%',
      onEnter: () => {
...
function initAIDelivery(section, ctx, cleanups) {
  ...
  gsap.set(cards, { opacity: 0.52, y: 14 });
  if (closer) gsap.set(closer, { opacity: 0.52, y: 10 });
  ...
  if (line) {
    tl.to(line, { scaleX: 1, duration: 0.72, ease: STORY_EASE_OUT }, 0);
  }
