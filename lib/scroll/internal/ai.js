...
function initAIStory(section, ctx, cleanups) {
  ...
  steps.forEach((step, index) => {
    ...
    if (num) {
      pushCleanup(
        cleanups,
        initCharReveal(num, ctx, { trigger: step, maxChars: 2, duration: 0.682 }),
      );
    }
...
function initAIGains(section, ctx, cleanups) {
  ...
  gains.forEach((gain, index) => {
    ...
    const play = () => {
      ...
      tl.to(gain, { opacity: 1, y: 0, duration: 0.68 }, at);
      ...
      if (valueInner) {
        ...
        tl.fromTo(gnum, { opacity: 0.4, y: 8 }, { opacity: 1, y: 0, duration: 0.68 }, at + 0.08);
      }
      ...
      if (copy) {
        tl.to(copy, { opacity: 1, y: 0, duration: 0.68 }, at + 0.22);
      }
    };
...
function initAIDelivery(section, ctx, cleanups) {
  ...
  if (lede) {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(lede, { opacity: 1, y: 0, duration: 0.68, ease: 'power2.out', delay: 0.1 });
      },
    });
    pushCleanup(cleanups, () => st.kill());
  }
  ...
  if (line) {
    ...
    pushCleanup(
      cleanups,
      initDrawLine(line, ctx, { trigger: block, axis: 'x', duration: 0.68 }),
    );
  }
  ...
  cards.forEach((card, index) => {
    ...
    if (cardCopy) {
      ...
      const st = ScrollTrigger.create({
        ...
        once: true,
        onEnter: () => {
          gsap.to(cardCopy, { opacity: 1, y: 0, duration: 0.68, ease: 'power2.out', delay: 0.14 });
        },
      });
      pushCleanup(cleanups, () => st.kill());
    }
  ...
  if (closer) {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(closer, { opacity: 1, y: 0, duration: 0.68, ease: 'power2.out' });
      },
    });
    pushCleanup(cleanups, () => st.kill());
  }
...
function initAIBand(section, ctx, cleanups) {
  ...
  pushCleanup(
    cleanups,
    initClipReveal(band, ctx, { direction: 'bottom', trigger: band, duration: 0.68 }),
  );
  ...
  if (btn) {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(btn, { opacity: 1, y: 0, duration: 0.68, ease: 'power2.out', delay: 0.16 });
      },
    });
    pushCleanup(cleanups, () => st.kill());
