...
/*
 * Colores del "text paint". Se leen de la paleta CSS para no volver a tener
 * una copia del dorado en JS; los literales son solo respaldo si la variable
 * falta o si esto corre sin document.
 */
function paintColor(varName, fallback) {
...
/*
 * Fallbacks alineados a la paleta actual (#f4f4f1 monocroma). Solo se usan si
 * la variable no resuelve o si esto corre sin document — en la practica estas
 * funciones se invocan siempre en cliente, dentro de callbacks de GSAP/scroll,
 * asi que la rama de fallback es inalcanzable en uso normal. Se mantienen
 * sincronizados igual, como pide CLAUDE.md para cualquier valor de respaldo.
 */
const MUTED = () => `rgb(${paintColor('--p-cream-rgb', '244 244 241')} / 0.15)`;
const ACTIVE = () => paintColor('--p-cream', '#f4f4f1');
const GOLD = () => paintColor('--role-accent', '#f4f4f1');
