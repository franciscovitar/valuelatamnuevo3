...
import { MOTION_SYSTEM, prefersReducedMotion } from '@/lib/motion/tokens';
...
export function initTextPaint(element, ctx, options = {}) {
  ...
  const delay = options.delay ?? 0.04;
  const stagger = options.stagger ?? MOTION_SYSTEM.wordStagger;
  const wordDuration = options.wordDuration ?? 0.58;
...
export function initProgressiveWrite(element, ctx, options = {}) {
  ...
  gsap.set(element, { opacity: 1 });
  gsap.set(units, { opacity: 0, y: options.y ?? MOTION_SYSTEM.minorDistance });
  ...
  const play = () => {
    ...
    tween = gsap.to(units, {
      opacity: 1,
      y: 0,
      duration: options.duration ?? MOTION_SYSTEM.reveal,
...
export function initFadeReveal(elements, ctx, options = {}) {
  ...
  const play = () => {
    ...
    tween = gsap.to(items, {
      ...
      y: 0,
      duration: options.duration ?? MOTION_SYSTEM.reveal,
      stagger: options.stagger ?? MOTION_SYSTEM.stagger,
      ease: MOTION_SYSTEM.easeOut,
      clearProps: 'transform,opacity',
    });
  };
