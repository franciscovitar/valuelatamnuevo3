...
import { pushCleanup, textRevealStart } from '@/lib/scroll/home/utils';
...
export function initHomeTextReveals(root, ctx, cleanups) {
  ...
  HOME_TEXT_TARGETS.forEach(({ sel, mode, mobileOnly }) => {
    ...
    const trigger = el.closest('.sec-head') || el.closest('.cta-grid > div:first-child') || el;
    const cleanup = initTextReveal(el, ctx, {
      mode,
      trigger,
      start: textRevealStart(ctx),
    });
