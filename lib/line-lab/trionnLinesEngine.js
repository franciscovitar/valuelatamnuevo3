...
const OUTER_X = [760, 420, 800, 900, 680, 260, 110, 320, 820, 900, 700, 260, 460, 740];
const MIDDLE_X = [710, 460, 750, 850, 630, 300, 150, 370, 770, 850, 650, 310, 510, 690];
const INNER_X = [660, 500, 700, 800, 580, 340, 190, 420, 720, 800, 600, 360, 560, 640];
  ...
  if (process.env.NODE_ENV !== 'production') {
    for (let i = 0; i < points.length; i += 1) {
      if (points[i][0] < SAFE_X_MIN - Y_MONOTONIC_TOLERANCE || points[i][0] > SAFE_X_MAX + Y_MONOTONIC_TOLERANCE) {
        console.warn(
          `[line-lab] ruta "${lineKey}" sale de SAFE_X en muestra ${i} (x=${points[i][0]}).`
        );
      }
      if (i === 0) continue;
...
export function resolveHeadWorldY(progress, lineKey) {
  ...
  const start = WORLD_Y[0] - HEAD_Y_OFFSET[lineKey];
  const end = WORLD_Y[WORLD_Y.length - 1];
  return lerp(start, end, headProgress);
}
...
export function resolveLineState(progress, lineKey) {
  ...
  const trailD = sliceRoute(samples, 0, headU);
  const trailPointCount = countTrailPoints(samples, 0, headU);
