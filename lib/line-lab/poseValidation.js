...
export function validateRoutes(routes = MASTER_ROUTES) {
  ...
  // Interval spreads should remain controlled and compact.
  ...
  for (let idx = 0; idx < CONVERGENCE_BEATS.length - 1; idx += 1) {
    ...
    if (spread < 72 || spread > 135) warnings.push(`Midpoint ${idx} spread fuera de [72,135]: ${spread.toFixed(3)}.`);
    if (spread >= 84 && spread <= 126) compactMidCount += 1;
  ...
  if (compactMidCount < 7) warnings.push(`Bundle poco compacto: solo ${compactMidCount}/9 midpoints en [84,126].`);
  ...
  for (let p = 0; p <= 1 + 1e-9; p += PROGRESS_STEP) {
    ...
    const cameraTop = resolveCameraTop(progress, { mode: 'home', profileKey: 'desktop' });
    if (cameraTop < prevCameraTop - 1e-9) warnings.push(`cameraTop disminuyó en p=${progress.toFixed(3)}.`);
    prevCameraTop = cameraTop;

    const o = resolveLineState(progress, 'outer', { profileKey: 'desktop' });
    const m = resolveLineState(progress, 'middle', { profileKey: 'desktop' });
    const i = resolveLineState(progress, 'inner', { profileKey: 'desktop' });
    ...
    if (spread > 135) warnings.push(`Spread global excede 135 en p=${progress.toFixed(3)} (${spread.toFixed(3)}).`);
    ...
    if (prevSpread !== null && Math.abs(spread - prevSpread) > 14) {
    ...
    [
      ...
      if (state.activeTrailPointCount <= 2 && progress > 0.1) warnings.push(`"${key}": activeTrailPointCount insuficiente en p=${progress.toFixed(3)}.`);
      if (state.headWorldY - state.activeStartWorldY > ACTIVE_WORLD_HEIGHT + 1e-6) warnings.push(`"${key}": active trail excede altura objetivo.`);
      prevHead[key] = state.headU;
      prevCount[key] = state.trailPointCount;
    });
  }

  // Convergence continuity guard using gesture offsets.
  for (let i = 0; i < INTERVAL_DANCE.length - 1; i += 1) {
    const dx = Math.abs(INTERVAL_DANCE[i + 1].aOffset - INTERVAL_DANCE[i].bOffset);
    if (dx > 220) warnings.push(`Salto de gesto excesivo entre intervalos ${i} y ${i + 1}.`);
  }

  // Persistencia final 0.96 vs 1
  const camera96 = resolveCameraTop(0.96, { mode: 'home', profileKey: 'desktop' });
  const camera1 = resolveCameraTop(1, { mode: 'home', profileKey: 'desktop' });
  if (Math.abs(camera96 - camera1) > 0.000001) warnings.push('cameraTop difiere entre 0.96 y 1.');

  LINE_KEYS.forEach((key) => {
    const a = resolveLineState(0.96, key, { profileKey: 'desktop' });
    const b = resolveLineState(1, key, { profileKey: 'desktop' });
    ...
    if (a.historyTrailD !== b.historyTrailD) warnings.push(`"${key}": historyTrailD difiere entre 0.96 y 1.`);
    if (!a.historyTrailD.trim()) warnings.push(`"${key}": historyTrailD final vacío.`);
    if (!a.activeTrailD.trim()) warnings.push(`"${key}": activeTrailD final vacío.`);
  });

  // Reversibilidad geométrica
  LINE_KEYS.forEach((key) => {
    const a = resolveLineState(0.37, key, { profileKey: 'desktop' });
    const c = resolveLineState(0.37, key, { profileKey: 'desktop' });
    if (
      a.historyTrailD !== c.historyTrailD ||
      a.activeTrailD !== c.activeTrailD ||
      a.headU !== c.headU ||
      a.tailU !== c.tailU ||
